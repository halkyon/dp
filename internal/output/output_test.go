...
func TestGetOutputFieldNames(t *testing.T) {
	t.Run("json with no query fields returns nil", func(t *testing.T) {
		got := GetOutputFieldNames("json", false, nil)
		assert.Nil(t, got)
	})

	t.Run("json with query fields returns those fields", func(t *testing.T) {
		got := GetOutputFieldNames("json", false, []string{"Name", "Price"})
		assert.Equal(t, []string{"Name", "Price"}, got)
	})

	t.Run("table narrow defaults", func(t *testing.T) {
		got := GetOutputFieldNames("table", false, nil)
		assert.Equal(t, []string{"Name", "Alias", "Status", "Location", "IP"}, got)
	})

	t.Run("table wide defaults", func(t *testing.T) {
		got := GetOutputFieldNames("table", true, nil)
		assert.Equal(t, []string{"Name", "Alias", "Status", "Power", "Location", "IP", "OS", "CPU", "Memory", "Storage", "Price"}, got)
	})

	t.Run("table with query fields", func(t *testing.T) {
		got := GetOutputFieldNames("table", false, []string{"Name", "Uptime"})
		assert.Equal(t, []string{"Name", "Uptime"}, got)
	})

	t.Run("csv narrow defaults", func(t *testing.T) {
		got := GetOutputFieldNames("csv", false, nil)
		assert.Equal(t, []string{"Name", "Alias", "Status", "Location", "IP"}, got)
	})

	t.Run("csv wide defaults", func(t *testing.T) {
		got := GetOutputFieldNames("csv", true, nil)
		assert.Equal(t, []string{"Name", "Alias", "Status", "Power", "Location", "IP", "OS", "CPU", "Memory", "Storage", "Price"}, got)
	})

	t.Run("raw defaults to Name", func(t *testing.T) {
		got := GetOutputFieldNames("raw", false, nil)
		assert.Equal(t, []string{"Name"}, got)
	})

	t.Run("raw with query fields", func(t *testing.T) {
		got := GetOutputFieldNames("raw", false, []string{"Name", "Alias"})
		assert.Equal(t, []string{"Name", "Alias"}, got)
	})
}
